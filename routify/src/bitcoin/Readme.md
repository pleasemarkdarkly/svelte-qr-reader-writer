# bitcoin-lib library abstractions
The following contains function helper names, various libraries which support the implementation, and the implementation and sequence functions to abstract the transaction type. Some of the following will be implemented client-side, and/or integrated with Firebase Cloud functions.
...
## Existing client/node libraries
...
function createTransaction(typei, txidi, outni, outputi, amounti, wifi, changeout, changeamt, inputvalue, networkInput){
	let NETWORK = networkInput === "testnet" ? bitcoin.networks.testnet : bitcoin.networks.bitcoin;
	// type1 indicate input type by using 1st character of address type spending from
	// examples 
	...
	//
	// spending from segwit addresses (p2sh or native) you must specify input value, this is the full value of the unspent output being spent which becomes the input in the new transaction
	// examples
...
// HD functions are not documented yet, still needs work to be easier to sync with trezor / mycelium and offer both witness and non-witness
